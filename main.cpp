...
int vest_realign(int argc,char* argv[]){
    ...
    if(args_realign.is_set(Opt_Realign::GFF)){ // if a gff is provided - make sure the anchoring nodes are not removed when fitting reads
        std::string out_gff_fname = args_realign.get_string(Opt_Realign::OUTPUT);
        out_gff_fname.append(".gff");
        msa.pre_fit_annotation(args_realign.get_string(Opt_Realign::GFF));
    }
